...
ch4 = load(joinpath(datadep"rffsps_v4", "emissions", "rffsp_ch4_emissions.csv")) |> 
    DataFrame |> @filter(_.year in collect(2020:2300)) |> @filter(_.sample == id) |> DataFrame
n2o = load(joinpath(datadep"rffsps_v4", "emissions", "rffsp_n2o_emissions.csv")) |> 
    DataFrame |> @filter(_.year in collect(2020:2300)) |> @filter(_.sample == id) |> DataFrame
co2 = load(joinpath(datadep"rffsps_v4", "emissions", "rffsp_co2_emissions.csv")) |> 
    DataFrame |> @filter(_.year in collect(2020:2300)) |> @filter(_.sample == id) |> DataFrame
...
t = Arrow.Table(joinpath(datadep"rffsps_v4", "pop_income", "rffsp_pop_income_run_$id.feather"))
...
# check death rate
pop_trajectory_key = (load(joinpath(datadep"rffsps_v4", "sample_numbers", "sampled_pop_trajectory_numbers.csv")) |> DataFrame).x
...
ypc1990 = load(joinpath(datadep"rffsps_v4", "ypc1990", "rffsp_ypc1990.csv")) |> 
                DataFrame |> 
